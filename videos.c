...
void curtirVideo(int idUsuarioLogado, int idProcurado) {
    ...
    // agora atualiza likes no arquivo de v?deos
    FILE *arq = fopen("videos.txt", "r");
...
void descurtirVideo(int idUsuarioLogado, int idProcurado) {
    ...
    // remove a curtida do usu?rio logado
    while (fscanf(arqCurtidas, "%d %d",
                  &usuarioLido,
                  &videoLido) == 2) {
        ...
        if (usuarioLido == idUsuarioLogado &&
            ...
            encontrouCurtida = 1; // n?o grava no temp = remove
        } else {
            fprintf(temp, "%d %d\n",
                    usuarioLido,
                    videoLido);
    ...
    // agora diminui o like no filmes.txt
    FILE *arq = fopen("videos.txt", "r");
...
void buscarPorCategoria(char categoriaBusca[]) {
	FILE *arq = fopen("videos.txt", "r");
	...
	if (arq == NULL) {
		printf("Erro ao abrir o arquivo de filmes e series!\n");
		return;
	}
	
	printf("\n--- Videos da Categoria: %s ---\n", categoriaBusca);
	
	while (fscanf(arq, "%d %s %s %d", 
	              &v.id, 
				  v.titulo, 
				  v.categoria, 
				  &v.likes) != EOF) {
		// compara a string da categoria
		if (strcmp(v.categoria, categoriaBusca) == 0) {
			printf("ID: %d\n", v.id);
			printf("Titulo: %s\n", v.titulo);
			printf("Categoria: %s\n", v.categoria);
			printf("Likes: %d\n", v.likes);
	...
	fclose(arq); // fecha arq para liberar o buffer
	
	if (!encontrado) {
		printf("Nenhum video encontrado nesta categoria.\n");
	}
}
