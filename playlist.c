...
void removerVideoPlaylist(int idPlaylistAlvo, int idVideoRemover, int idUsuarioLogado) {
	...
	if (!removeu) {
		printf("Esse video nao existe na playlist.\n");
		return;
	}
	          ...
	          playlists[i].lista_videos) != EOF) {
	          	...
	          	if (playlists[i].id_p == idPlaylistAlvo &&
	          	    playlists[i].id_u == idUsuarioLogado) {
	          	    	...
	          	    	while (token != NULL) {
	          	    		if (atoi(token) == idNovoVideo) {
	          	    			printf("Esse video ja esta na playlist.\n");
	          	    			encontrada = - 1;
	          	    			break;
							  }
						  ...
						  // formata o novo id e anexa usando strcat
